...
class Investor:
    ...
    def prepare_data(cls, index_name):
        ...
        # data = pd.read_excel("/home/nischal/Documents/GitHub/IStrat/" + index_name + ".xlsx")
        ...
        cls.years = sorted(data["Date"].apply(lambda x: x.year).unique().tolist())
    ...
    def calculate_return_ratio(self):
        ...
        roi_ratio = self.data.loc[num_el - 1, "Asset_Value"] / self.data.loc[num_el - 1, "Total_Investment"]
        return np.round(roi_ratio, 2)

    def calculate_asset_value(self, stock_price):
    ...
    def log_data(self, idx, stock_price, stocks_acquired):
        ...
        self.data.loc[idx, "Asset_Value"] = self.calculate_asset_value(stock_price)
    ...
    def invest_monthly(
        ...
        apply_boost=False,
        boost_perc=1e-12,
        apply_yearly_increment=False,
        increment_in_years=None,
        incr_fac=None,
    ):
        """
        ...
        if apply_yearly_increment:
            ...
            years_multiplication_factor = {year: incr_fac ** (k) for k, year in enumerate(incremental_years)}
...
p3 = P3.invest_monthly(
    amount=amnt,
    apply_yearly_increment=True,
    increment_in_years=inc_every,
    incr_fac=1 + 0.01 * inc_perc,
)
...
if index_name != "snp":
    ...
    plt.plot(
        p1["Date"],
        p1["Asset_Value"] / scale_fac,
        "r",
        label=f"Strategy 1 (Invest ₹ {amnt} Monthly, CAGR = {format(P1.calculate_cagr(),'.2f')} %)",
    )
...
plt.plot(
    p1["Date"],
    p1["Total_Investment"] / scale_fac,
    "--r",
    label="Strategy 1: Total Investment",
)
plt.plot(
    p2["Date"],
    p2["Asset_Value"] / scale_fac,
    "b",
    label=f"Strategy 2 (Apply Monthly Boosting, CAGR = {format(P2.calculate_cagr(),'.2f')} %)",
)
plt.plot(
    p2["Date"],
    p2["Total_Investment"] / scale_fac,
    "--b",
    label="Strategy 2: Total Investment",
)
plt.plot(
    p3["Date"],
    p3["Asset_Value"] / scale_fac,
    "g",
    label=f"Strategy 3 ({inc_perc}% increase Investment every {inc_every} year, CAGR = {format(P3.calculate_cagr(),'.2f')} %)",
)
plt.plot(
    p3["Date"],
    p3["Total_Investment"] / scale_fac,
    "--g",
    label="Strategy 3: Total Investment",
)
